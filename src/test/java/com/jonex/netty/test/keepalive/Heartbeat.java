...
public class Heartbeat extends ChannelHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if(msg instanceof  KeepAliveMessage){
            ...
            System.out.println(ctx.channel().remoteAddress() + " Say : sn=" + keepAliveMessage.getSn()+",reqcode="+keepAliveMessage.getCode());
            keepAliveMessage.setCode(Constants.RET_CODE);
            if(Utils.notEmpty(keepAliveMessage.getSn())&&keepAliveMessage.getCode()==1){
                ctx.channel().writeAndFlush(msg);
                // 失败计数器清零
                unRecPingTimes = 0;
                if(localMsgInfo.get()==null){
                    KeepAliveMessage localMsg = new KeepAliveMessage();
                    localMsg.setSn(keepAliveMessage.getSn());
